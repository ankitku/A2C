...
(defun mk-dfa-events (name states alphabet start accept transition-fun)
  (b* ((d-state (gen-symb "~a-state" name))
       ...
       ((unless (second (acl2s-compute `acl2s::(,d-fp (quote ,transition-fun)))))
        (error-and-reset "incorrect transition function, check syntax" d-state))
       (doms (strip-cars transition-fun))
       (res (equal doms (remove-duplicates-equal doms)))
       ((unless res) (error-and-reset "Domain of the transition function is not unique" d-state))
       (res (itest?-query
             `acl2s::(=> (,(gen-sym-pred d-tdom) dom)
                         (in dom (quote ,doms)))))
       ((when (car res)) (error-and-reset (format nil "Missing ~a from domain of the transition function" (mapcar #'cadar (cadadr res))) d-state))
...
(defun query-equivalence (dfa1-name dfa2-name)
  (b* ((dn (gen-symb "~a-wordp" dfa1-name))
       (dfa1 (gen-symb-const dfa1-name))
       (dfa2 (gen-symb-const dfa2-name))
       ((unless (boundp dfa1))
        (cons nil (format nil "Undefined DFA ~a" dfa1-name)))
       ((unless (boundp dfa2))
          (cons nil (format nil "Undefined DFA ~a" dfa2-name)))
